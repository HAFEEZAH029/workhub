      ...
      <div className="mx-auto max-w-7xl">
        ...
        >
          {features.map(({ icon: Icon, iconBg, iconColor, title, description }) => (
            ...
            >
              <span className={`grid size-14 place-items-center rounded-2xl ${iconBg}`}>
                <Icon className={`size-6 ${iconColor}`} strokeWidth={2} />
              </span>
              <h3 className="mt-5 text-xl font-bold text-app-neutral">{title}</h3>
              <p className="mt-2 max-w-xs text-sm leading-6 text-app-neutral/65">{description}</p>
            </motion.div>
          ))}
        </motion.div>
      </div>
